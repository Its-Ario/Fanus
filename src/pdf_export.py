...
def formal_scores(student) -> tuple[FormalScore, ...]:
    ...
    return tuple(
        FormalScore(row.subject_name, row.exam.term, row.score, row.exam.max_score, row.exam.exam_date)
...
def mock_trends(student) -> tuple[MockTrend, ...]:
    ...
    return tuple(
        MockTrend(values[0].exam.name, values[0].exam.exam_date, round(
            sum(row.score * 100.0 / row.exam.max_score for row in values) / len(values), 1
        ))
...
def attendance_summary(student) -> AttendanceSummary:
    rows = AttendanceRecord.select(AttendanceRecord.status, fn.COUNT(AttendanceRecord.id).alias("count")).where(
...
def focus_note(student, plan) -> str:
    ...
    return (
        ...
        if frequent else "برای هفته پیش رو، پیگیری منظم برنامه توصیه می‌شود."
    )
...
def _footer(painter: QPainter, width: int, height: int) -> None:
    ...
    _text(painter, QRectF(_MARGIN, height - _MARGIN + 7, width - 2 * _MARGIN, 20),
          f"تاریخ تهیه: {jalali_date(date.today())}", 9)
...
def export_weekly_plan_pdf(plan, path: str) -> None:
    ...
    try:
        ...
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 22),
              f"بازه برنامه: {jalali_date(plan.start_date)} تا {jalali_date(plan.end_date)}", 11)
        y += 32
        ...
        for column in range(8):
            _line(painter, _MARGIN + column * col_width, y, _MARGIN + column * col_width, y + grid_height)
        for row in range(rows + 2):
            _line(painter, _MARGIN, y + row * row_height, width - _MARGIN, y + row * row_height)
        for day in range(7):
            left = _MARGIN + day * col_width
            _text(painter, QRectF(left + 3, y + 2, col_width - 6, row_height - 4), DayOfWeek.PERSIAN_NAMES[day], 11, True, Qt.AlignCenter)
            for row, session in enumerate(sessions[day]):
                cell = QRectF(left + 2, y + (row + 1) * row_height + 2, col_width - 4, row_height - 4)
                painter.fillRect(cell, QColor(_COLORS[catalog.type_for(session.subject_name)]))
                _text(painter, cell, f"{session.subject_name}\n{session.start_time}–{session.end_time}\n{session.session_type}", 9, False, Qt.AlignCenter)
        y += grid_height + 16
        ...
        y += 44
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 22),
              f"مجموع ساعات موظف: {to_persian_digits(round(target * 7, 1))} ساعت | مجموع برنامه‌ریزی‌شده: {to_persian_digits(total)} ساعت", 11, True)
        y += 30
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 38), "یادداشت آموزشی: " + focus_note(plan.student, plan), 11)
        signature_y = _signature_y(height)
        _text(painter, QRectF(_MARGIN, signature_y, (width - 2 * _MARGIN) / 2, 30), "امضای مشاور: ........................", 11)
        _text(painter, QRectF(width / 2, signature_y, (width - 2 * _MARGIN) / 2, 30), "امضای والدین: ........................", 11)
...
def export_academic_summary_pdf(student, path: str) -> None:
    ...
    try:
        ...
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 24), "نمرات رسمی (آخرین نمره هر درس در هر نوبت)", 13, True)
        y += 32
        ...
        def table_row(values, header=False):
            nonlocal y
            painter.fillRect(QRectF(_MARGIN, y, width - 2 * _MARGIN, row_height), QColor("#E2E8F0" if header else "#FFFFFF"))
            for index, value in enumerate(values):
                cell_width = (width - 2 * _MARGIN) * widths[index]
                _text(painter, QRectF(lefts[index] + 4, y + 2, cell_width - 8, row_height - 4), value, 10, header, Qt.AlignCenter if index > 0 else Qt.AlignRight)
        ...
        if scores:
            for score in scores:
                table_row((score.subject, score.term, to_persian_digits(f"{score.score:g}/{score.ceiling:g}"), jalali_date(score.exam_date)))
        else:
            table_row(("نمره رسمی ثبت نشده است.", "—", "—", "—"))
        y += 18
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 24), "روند عملکرد آزمون آزمایشی", 13, True)
        y += 30
        trends = mock_trends(student)
        if trends:
            for trend in trends:
                _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 24),
                      f"{trend.name} — {jalali_date(trend.exam_date)}: {to_persian_digits(trend.percentage)}٪", 11)
                y += 26
        else:
            _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 24), "آزمون آزمایشی ثبت نشده است.", 11)
            y += 28
        attendance = attendance_summary(student)
        y += 12
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 24), "حضور و غیاب", 13, True)
        y += 27
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 36),
              ...
              "به دلیل ثبت‌نشدن همه روزهای حضور، نرخ قابلیت اتکای حضور قابل محاسبه نیست.", 10)
        y += 55
        _text(painter, QRectF(_MARGIN, y, width - 2 * _MARGIN, 32),
              "یادداشت‌های محرمانه مشاور در این گزارش وجود ندارد و به هیچ‌وجه صادر نمی‌شود.", 10, True)
        _footer(painter, width, height)
    finally:
        painter.end()
