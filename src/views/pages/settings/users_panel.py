...
class _UsersModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        if not index.isValid():
            return None
        user = self.rows[index.row()]
        values = (
            user.full_name,
            user.username,
            {"assistant": "معاون", "counselor": "مشاور", "principal": "مدیر"}.get(
                user.role, user.role
            ),
            "مدیر تنظیمات" if user.can_manage_users else "—",
        ...
        if role == Qt.TextAlignmentRole:
            return Qt.AlignRight | Qt.AlignVCenter
        return user if role == Qt.UserRole else None
...
class UsersPanel(QWidget):
    def __init__(self, actor, editable, parent=None):
        ...
        self.toggle = SecondaryButton("فعال/غیرفعال")
        self.toggle.clicked.connect(self.toggle_selected)
        self.toggle.setHidden(not editable)
        header.addWidget(self.toggle)
        self.add = PrimaryButton("کاربر جدید")
        self.add.clicked.connect(self._add)
        self.add.setHidden(not editable)
        header.addWidget(self.add)
        layout.addLayout(header)
        self.inactive = QCheckBox("نمایش کاربران غیرفعال")
    ...
    def reload(self):
        ...
        if not self.inactive.isChecked():
            query = query.where(User.is_active)
        self.model.set_rows(query)

    def _add(self):
        if UserDialog(self.actor, parent=self).exec_():
            self.reload()
    ...
    def _edit(self, index):
        if (
            self.editable
            and index.isValid()
            and UserDialog(self.actor, index.data(Qt.UserRole), self).exec_()
        ):
            self.reload()
    ...
    def toggle_selected(self):
        ...
        if user:
            try:
                set_user_active(self.actor, user, not user.is_active)
                self.reload()
            except Exception:
                pass
