...
def migrate(migrator, database, fake=False, **kwargs):
    ...
    duplicates = database.execute_sql(
        "SELECT grade_level, major, code, academic_year, COUNT(*) FROM classroom GROUP BY 1,2,3,4 HAVING COUNT(*) > 1"
    ).fetchall()
    if duplicates:
    ...
    for identifier, grade, major, code in rows:
        ...
        name = (
            f"{prefix} - {code}"
            if major == "\u0639\u0645\u0648\u0645\u06cc/\u0645\u0639\u0627\u0631\u0641"
            else f"{prefix} {major} - {code}"
        )
        database.execute_sql("UPDATE classroom SET name = ? WHERE id = ?", (name, identifier))
    database.execute_sql(
        "CREATE UNIQUE INDEX IF NOT EXISTS classroom_grade_level_major_code_academic_year ON classroom (grade_level, major, code, academic_year)"
    )
