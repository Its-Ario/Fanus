"""Study-plan engine: budget math, grid exclusions, solver invariants, end-to-end."""
...
def test_budget_fits_time_and_never_starves_a_graded_subject():
    ...
    blocks = budget.weekly_blocks(subjects, coeffs, weaknesses, 35.0, 1.5)

    assert sum(blocks.values()) * 1.5 <= 35.0 + 1e-9
...
def test_solver_output_passes_the_hard_constraint_checker():
    ...
    weaknesses = {s: 1.5 for s in subjects}
    blocks = budget.weekly_blocks(subjects, coeffs, weaknesses, 30.0, 1.5)
...
def test_solver_is_deterministic():
    ...
    coeffs = {s: catalog.coefficient_for(s, AcademicMajor.MATH) for s in subjects}
    weaknesses = {s: 1.5 for s in subjects}
    blocks = budget.weekly_blocks(subjects, coeffs, weaknesses, 24.0, 1.5)
    windows = grid.free_windows(SCHOOL_DAYS, SCHOOL_HOURS)
    slots = grid.candidate_slots(windows, 90, SCHOOL_DAYS)
...
def _seed(tmp_path, *, daily_hours=5.0):
    ...
    SchoolProfile.create(
        id=1, school_name="دبیرستان نمونه", academic_year="1404-1405", type="high"
    )
    ...
    for subject, score in (("زیست‌شناسی ۲", 9.0), ("شیمی ۲", 11.0), ("ریاضی ۲", 8.0)):
        AcademicGrade.create(
            student=student, subject_name=subject, score=score, max_score=20.0
        )
